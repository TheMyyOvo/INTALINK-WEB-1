    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
            ...
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['dataConfiguration:dataModel:add']"
        >{{ $t('btn.add') }}</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['dataConfiguration:dataModel:remove']"
      ...
      <el-col :span="1.5">
        <el-button
            ...
            icon="Upload"
            @click="handleImport"
            v-hasPermi="['dataConfiguration:dataModel:import']"
...
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column :label="$t('modelManagement.table_Number')" align="center" prop="dataTableCount">
        <template #default="{row}">
<!--          <el-input v-if="row.status" v-model="row.dataTableCount"></el-input>-->
      ...
      <el-table-column :label="$t('modelManagement.operation')" width="300" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button  link type="primary" icon="Upload" @click="handleImportPDM(scope.row)" v-hasPermi="['dataConfiguration:dataModel:import']">{{ $t('btn.PDMImport') }}</el-button>
          <el-button v-if="!scope.row.status" link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['dataConfiguration:dataModel:edit']">{{ $t('btn.edit') }}</el-button>
          <el-button v-else link type="primary" icon="Edit" @click="handleSaveUpdate(scope.row)" v-hasPermi="['dataConfiguration:dataModel:save']">{{ $t('btn.save') }}</el-button>

          <el-button link type="primary" icon="Edit" @click="dataTableConfig(scope.row)" v-hasPermi="['dataConfiguration:dataModel:edit']">{{ $t('btn.Table_configuration') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
<!--Excel导入-->
    <el-dialog :title="upload.title" v-model="upload.open" width="400px">
      ...
      >
        ...
        <div class="el-upload__text">{{ $t('PublicVariable.Drag_files_tip') }}<em>{{ $t('btn.Click_to_upload') }}</em></div>
        <template #tip>
          <div class="el-upload__tip text-center">

            <span>{{ $t('PublicVariable.importing_xls_tip') }}。</span>
            <el-link type="primary" :underline="false" style="font-size:12px;vertical-align: baseline;" @click="importTemplate">{{ $t('btn.Download_template') }}</el-link>
          </div>
        </template>
      </el-upload>
      <template #footer>
    ...
    <el-dialog :title="uploadPDM.title" v-model="uploadPDM.openPDM" width="400px">
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button @click="uploadPDM.openPDM = false">{{ $t('btn.cancel') }}</el-button>
        </div>
      </template>
    </el-dialog>
...
<script  setup>
  ...
  const upload = reactive({
    ...
    // 上传的地址
    url: import.meta.env.VITE_APP_BASE_API + "/configoperations/dataModel/modelImport"
  });
  /*** 用户导入PDM参数 */
  const uploadPDM = reactive({
    ...
    headers: { Authorization: "Bearer " + getToken() },
    // 上传的地址
    url: import.meta.env.VITE_APP_BASE_API + "/configoperations/pdm/upload/" + importmodelid.value
  });
  onMounted(() => {
    getList();
  ...
  function dataTableConfig(row){
    ...
    router.push({
      ...
      query:{dataModelId,isModel:true}
    })
  }
  /** 删除按钮操作 */
  function handleDelete(row) {
    ...
    let showNameList=seleData.filter(item => item.dataTableCount != '0').map(ele=>{ return ele.dataModelName}).join()
    // console.log(showNameList)
    if(showNameList==''){
      showContent=i18n.global.t('PublicVariable.Delete_operation_tip')
    }else{
      showContent=`${showNameList}${i18n.global.t('PublicVariable.Already_bound')}`
    }
  ...
  function handleImport() {
    upload.title = i18n.global.t('modelManagement.Model_import');
    upload.open = true;
  }
  /** 导入PDM按钮操作 */
  function handleImportPDM(row) {
    uploadPDM.title = i18n.global.t('btn.PDMImport');
    uploadPDM.openPDM = true;
    importmodelid.value=row.dataModelId
    uploadPDM.url=import.meta.env.VITE_APP_BASE_API + "/configoperations/pdm/upload?modelId=" + importmodelid.value

  };
  /** 下载模板操作 */
  function importTemplate() {
    proxy.download("configoperations/dataModel/importTemplate", {
    }, i18n.global.t('PublicVariable.Data_import_template')`.xlsx`);
  };
  ...
  const handleFileSuccess = (response, file, fileList) => {
    ...
    proxy.$alert("<div style='overflow: auto;overflow-x: hidden;max-height: 70vh;padding: 10px 20px 0;'>" + response.msg + "</div>", i18n.global.t('PublicVariable.Import_Results'), { dangerouslyUseHTMLString: true });
    getList();
  };
  /** PDM文件上传成功处理 */
